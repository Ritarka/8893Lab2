...
{
transaction<16> tr(17899520);
aesl_fh.read(AUTOTB_TVOUT_PC_fm, tr.p, tr.tbytes);
if (little_endian()) { tr.reorder(); }
tr.send<2>((char*)__xlx_apatb_param_input_feature_map, 2826240, 0);
tr.send<2>((char*)__xlx_apatb_param_output_feature_map, 15073280, 2826240);
}
#else
try {
...
if (__xlx_apatb_param_input_feature_map) {
  tr.import<2>((char*)__xlx_apatb_param_input_feature_map, 2826240, 0);
}
__xlx_offset_byte_param_output_feature_map = 2826240*2;
if (__xlx_apatb_param_output_feature_map) {
  tr.import<2>((char*)__xlx_apatb_param_output_feature_map, 15073280, 0);
}
...
unsigned char *pos = (unsigned char*)__xlx_apatb_param_layer_bias + i * 2;
std::string s = formatData(pos, 16);
aesl_fh.write(AUTOTB_TVIN_wt, s);
}
}
...
{
aesl_fh.write(AUTOTB_TVIN_layer_bias, begin_str(AESL_transaction));
{
auto *pos = (unsigned char*)&__xlx_offset_byte_param_layer_bias;
aesl_fh.write(AUTOTB_TVIN_layer_bias, formatData(pos, 32));
}
  tcl_file.set_num(1, &tcl_file.layer_bias_depth);
aesl_fh.write(AUTOTB_TVIN_layer_bias, end_str());
}
