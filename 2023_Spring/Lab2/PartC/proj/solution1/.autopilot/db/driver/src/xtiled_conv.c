...
void XTiled_conv_Set_layer_bias(XTiled_conv *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XTiled_conv_WriteReg(InstancePtr->Control_BaseAddress, XTILED_CONV_CONTROL_ADDR_LAYER_BIAS_DATA, (u32)(Data));
    XTiled_conv_WriteReg(InstancePtr->Control_BaseAddress, XTILED_CONV_CONTROL_ADDR_LAYER_BIAS_DATA + 4, (u32)(Data >> 32));
}

u64 XTiled_conv_Get_layer_bias(XTiled_conv *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XTiled_conv_ReadReg(InstancePtr->Control_BaseAddress, XTILED_CONV_CONTROL_ADDR_LAYER_BIAS_DATA);
    Data += (u64)XTiled_conv_ReadReg(InstancePtr->Control_BaseAddress, XTILED_CONV_CONTROL_ADDR_LAYER_BIAS_DATA + 4) << 32;
    return Data;
}
